...
Subject ID: `r toString((params$ISG %>% filter(Sample == cur_sample))['Sample ID'][[1]])`

Sample name: `r toString((params$ISG %>% filter(Sample == cur_sample))['Sample info'][[1]])`
...
tmp = (params$ISG) %>% 
  mutate(cur_label = case_when(Sample == cur_sample ~ sub('\\(.*$', '', `Sample info`), 
                               Sample == extra_label ~ sub('\\(.*$', '', `Sample info`))) %>%
