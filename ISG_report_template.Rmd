...
```{r}
# get the Sample ID and Subject info
tmp <- params$ISG %>% filter(Sample == cur_sample)
if (is.na(tmp$Visit)){
  sample_ID = tmp['Sample ID'][[1]]
  
} else {
  tmp <- params$ISG %>% filter(`Subject ID` == tmp$`Subject ID` & !is.na(Visit))
  sample_ID = paste0(paste(tmp$Visit,tmp$`Sample ID`, sep=': '), collapse = ', ')
}
subject = tmp['Sample info'][[1]]

```

Sample ID: `r toString(sample_ID)`

Subject: `r toString(subject)`
...
tmp = (params$ISG) %>% 
  mutate(cur_label = case_when((Sample == cur_sample) & (!is.na(label)) ~ label, 
                               (Sample == cur_sample) & (is.na(label)) ~ `Subject ID`,
                               (Sample == extra_label) & (!is.na(label)) ~ label, 
                               (Sample == extra_label) & (is.na(label)) ~ `Subject ID`)) %>%
  mutate(label = case_when((Sample != cur_sample) & (Sample != extra_label) ~ label))
...
tmp = params$ISG
tmp = tmp %>% filter(Sample == cur_sample)
tab2 = tibble(`ISG score (geomean)` = tmp$geomean,
              `ISG score (zscore)` = tmp$zscore)
