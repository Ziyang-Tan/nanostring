...
dat <- bind_rows(dataList) %>%
  mutate(Group = if_else(is.na(Group), 'Patients', Group)) %>%
...
datCorrectedFiltered <- datCorrected %>% filter(!batch %in% c('EXP-21-DN5206', 'EXP-21-DN5207','EXP-21-DN5208','EXP-21-DN5209',
                                                              'EXP-21-DN5210','EXP-21-DN5211','EXP-21-DN5212','EXP-21-DN5214'))
NFkb <- geomean_score(datCorrectedFiltered, NFkb_panel) %>%
  add_column(zscore = (zscore_score(datCorrectedFiltered, NFkb_panel))$zscore)
